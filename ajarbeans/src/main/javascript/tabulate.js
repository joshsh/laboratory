...
//
// SVN ID: $Id: tabulate.js 2717 2007-01-30 21:44:03Z jambo $
//
...
// JJS
kb.register('foaf', "http://xmlns.com/foaf/0.1/")
...
function label(x, trimSlash) {
	...
	return 'collection: ' + x.elements.length;
    }
...
function propertyTable(subject, table) {
...

			...
			return function(display){
			    ...
			    small.innerHTML = '';
			    if (display == 'none') {
				small.appendChild(AJARImage(Icon.src.icon_expand, 'more', 'See all'))
				small.appendChild( document.createTextNode((k-dups-n) + ' more...'));
			    } else {
				small.appendChild(AJARImage(Icon.src.icon_shrink, '(shrink)'))
...
function addButtonCallbacks(target, uri) {
    var fireOn = Util.uri.docpart(uri.uri)
    log.debug("Button callbacks for " + fireOn + " added")
    var makeIconCallback = function (icon) {
	return function (req) {
	    if (!target || req.termType != "symbol") {
		return false
	    }
	    if (uri.termType != "symbol") { return true }
	    req = Util.uri.docpart(req.uri)
...
function setSelected(node, newValue) {
    ...
    if (newValue) {
	    cla += ' selected'
	    if (cla.match('pred') || cla.match('obj')) setSelectedParent(node,1)
	    selection.push(node)
	    fyi("Selecting "+node)
	    var source
...
function outline_expand(p, subject) {
    var subjuri = subject.uri?kb.sym(Util.uri.docpart(subject.uri)):subject
    var sources = false
    
    function expand(uri, requestedBy) {
	var udoc = uri.uri?kb.sym(Util.uri.docpart(uri.uri)):uri
	if ((uri.sameTerm(subject)
	     || kb.anyStatementMatching(udoc,kb.sym('tab',"requestedBy"),
					subjuri))
...
function SourceWidget() {
	...
	var cb = function (u, r) {
	    ...
	    if (!u.uri) { return true }
	    var udoc = kb.sym(Util.uri.docpart(u.uri))
	    if (udoc.sameTerm(term)) {
		var req=kb.statementsMatching(udoc, kb.sym('tab', 'request'))
		var lstat = kb.the(req.pop().object, kb.sym('tab','status'))
	...
	if (u.uri) {
		...
		var rbtn = appendAccessIcon(icon,udoc)
		...
		sf.addCallback('refresh',function (u, r) {
				   ...
				   if (!u.uri) { return true }
				   var udoc2 = kb.sym(Util.uri.docpart(u.uri))
...
function AJAR_initialisePage()
{
    ...
    else
    {
    	links = { // JJS
                  ...
                  'http://dig.csail.mit.edu/2005/ajar/ajaw/data#Tabulator':'The tabulator project',
    			  'http://dig.csail.mit.edu/data#DIG':'Decentralised Information Group',
    			  'http://www.w3.org/data#W3C':'W3C  (large: includes all specs and groups)',
    			  'http://www.w3.org/2000/04/mem-news/groups.rdf':'W3C groups (Member-only data)',
    			  'http://www.w3.org/2001/04/roadmap/w3c.xml':'W3C roadmap data (old)',
    			  'http://www.w3.org/2005/04/swls/BioDash/Demo/gsk':'Life Sciences demo data: GSK3-Beta Drug Target' }
    	for (q in links)
    	{
    		/*var tr = document.createElement('tr')
    		var td = document.createElement('td')
